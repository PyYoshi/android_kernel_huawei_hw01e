...
#endif
/***** Orientation ***/
static ssize_t akm8975_set_cal_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
       int err = 0;
	err = sprintf(buf, "%d\n", calibration_value);
...
static ssize_t akm8975_set_cal_store(
	struct device *dev, struct device_attribute *attr,
	char const *buf, size_t count)
{ 
...
static struct device_attribute akm8975_attributes[] = {
	...
	__ATTR(set_cal,  0664, akm8975_set_cal_show,  akm8975_set_cal_store),
#ifdef AKM8975_DEBUG_IF
...
{
    struct akm8975_data *akm = container_of(
		handler, struct akm8975_data, early_suspend);
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /*board id dynomic adapt layout*/
    if(!is_compass_sensor_layout_top())
    {
        s_akm->layout = 7;
    }
    else
    {
        dev_err(&client->dev, "%s: Get layout failed, set default layout!.", __func__);
        s_akm->layout = 3;
 ...
 /* IRQ */
      ...
      {
	    client->irq = pdata->gpio_config_interrupt(1);
          if(client->irq<0)
          {
   ...
   #ifdef CONFIG_HAS_EARLYSUSPEND
	s_akm->early_suspend.suspend = akm8975_early_suspend;
	s_akm->early_suspend.resume = akm8975_early_resume;
	register_early_suspend(&s_akm->early_suspend);
    #endif
...
exit8:
    #ifdef CONFIG_HAS_EARLYSUSPEND
    unregister_early_suspend(&s_akm->early_suspend);
    #endif
exit7:

...
/* disabled IS_INVALID_TOUCH_POINT because of unknown wy check*/
/*
...
/* modify for u8600 */
                    ...
                    /* remove BTN_TOUCH report when finger is down for a bug which works fine with 9 fingers but not with 10. */
...
static int tp_read_input_name(void)
{
	...
	query_i2c_msg_name[0].buf = &fd_34.controlBase;
...
static ssize_t synaptics_vendor_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret = 0;
	sprintf(buf, "%s\n", "synaptics_rmi_1564");
...
static DEVICE_ATTR(vendor, S_IRUGO, synaptics_vendor_show, NULL);
static int touch_sysfs_init(void)
{
	...
	ret = sysfs_create_file(touch_kobj, &dev_attr_vendor.attr);
	if (ret) {
...
/*unify atmel platform_data struct and synaptics platform_data struct*/
    ...
    struct tp_i2c_platform_data *touch_pdata = NULL;
...
/* delete one line,needn't control avdd in driver */
    ...
    {
        ret = touch_pdata->power_on(&client->dev,SYNAPTICS_POWER_NAME);
    }    
    if(ret)
    {
    ...
    {
        client->irq = touch_pdata->touch_gpio_config_interrupt(1);
        if (client->irq<0) 
        {
            client->irq = 0;
            dev_err(&client->dev, "irq gpio reguest failed use poll\n");
        }
    }
...
/* Clear touch screen interrupt status register before irq_handler ready, let tp release irq pin */
	...
	return 0;
/*delete one line for gpio unite*/
/*we don't use the CONFIG_HUAWEI_TOUCHSCREEN_EXTRA_KEY so delete all the CONFIG_HUAWEI_TOUCHSCREEN_EXTRA_KEY code*/
...
/*unify atmel platform_data struct and synaptics platform_data struct*/
    struct tp_i2c_platform_data *touch_pdata = NULL;
    touch_pdata = client->dev.platform_data;;
	sysfs_remove_file(touch_kobj, &dev_attr_vendor.attr);
